...
<template>
  ...
  >
    <div class="relative aspect-[3/4] bg-gradient-to-br from-ink-800 to-ink-850 flex items-center justify-center overflow-hidden">
      <img
        v-if="book.cover_url"
        :src="book.cover_url"
        :alt="book.title"
      ...
      />
      <BookOpen v-else class="w-10 h-10 text-ink-300" :stroke-width="1.5" />
    ...
    <div class="p-2.5 flex-1 flex flex-col">
      ...
      <p class="text-xs text-ink-300 line-clamp-1 mt-0.5">{{ book.author || '佚名' }}</p>
      <p v-if="showMeta !== false && book.file_size" class="text-[10px] text-ink-300 mt-1.5">
