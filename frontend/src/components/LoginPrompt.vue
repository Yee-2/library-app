...
<template>
  ...
  >
    ...
    >
      <div class="bg-white w-[85vw] max-w-sm rounded-2xl p-6 shadow-2xl text-center">
        ...
        <h3 class="text-lg font-semibold text-ink-900 mb-2">需要登录</h3>
        <p class="text-sm text-ink-400 mb-6">登录后即可体验完整功能</p>
        <div class="flex gap-3">
          <button
            @click="emit('close')"
            class="flex-1 h-11 rounded-xl border border-ink-200 text-ink-8000 text-sm font-medium hover:bg-ink-50 transition"
          >取消</button>
          <button
            @click="goLogin"
            class="flex-1 h-11 rounded-xl bg-gradient-to-r from-primary-600 to-accent-600 text-white text-sm font-medium shadow-lg shadow-md hover:shadow-purple-500/40 transition"
          >去登录</button>
        </div>
      </div>
    </div>
