...
import {
  listFollowers, listFollowing, unfollowUser
} from '@/lib/books'
...
<template>
  <div class="max-w-3xl mx-auto px-4 py-6">
    <div class="flex items-center gap-2 mb-4">
      ...
      <h1 class="text-xl font-bold">{{ type === 'followers' ? '粉丝' : '关注' }}</h1>
    </div>
    <div v-if="loading" class="text-center text-slate-500 py-8">加载中…</div>
    <div v-else-if="list.length === 0" class="text-center text-slate-500 py-10">还没有人</div>
    <div v-else class="space-y-2">
      <div v-for="item in list" :key="item.follower_id || item.followee_id"
        ...
        <div class="w-10 h-10 rounded-full bg-gradient-to-br from-brand-400 to-brand-600 text-white flex items-center justify-center font-medium">
          {{ ((item.profiles as any)?.username || '?')[0].toUpperCase() }}
        </div>
        <div class="flex-1">
