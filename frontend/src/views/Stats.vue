...
<template>
  <div class="max-w-3xl mx-auto px-4 py-6">
    <div class="flex items-center gap-2 mb-4">
      ...
      <h1 class="text-xl font-bold">阅读统计</h1>
    </div>
    ...
    <div v-else>
      ...
      <div class="grid grid-cols-2 gap-3 mb-4">
        <div class="card p-4">
          ...
          <div class="text-2xl font-bold text-brand-600 mt-1">{{ fmtTime(totalSeconds) }}</div>
        </div>
        <div class="card p-4">
          <div class="text-xs text-slate-500">日均阅读</div>
          <div class="text-2xl font-bold text-brand-600 mt-1">{{ fmtTime(avgSecondsPerDay) }}</div>
        </div>
        <div class="card p-4">
          <div class="text-xs text-slate-500">累计字数</div>
          <div class="text-2xl font-bold text-brand-600 mt-1">{{ totalWords.toLocaleString() }}</div>
        </div>
        <div class="card p-4">
          <div class="text-xs text-slate-500">阅读会话</div>
          <div class="text-2xl font-bold text-brand-600 mt-1">{{ totalSessions }}</div>
        </div>
      </div>

      <!-- 图表 -->
      <div class="card p-4 mb-4">
        <h3 class="font-semibold text-sm mb-2">每日阅读分钟数</h3>
        <div class="h-56">
      ...
      <!-- 书目排行 -->
      <div class="card p-4">
        <h3 class="font-semibold text-sm mb-2">藏书（{{ books.length }}）</h3>
        <div v-if="books.length === 0" class="text-xs text-slate-400 py-3 text-center">还没有书</div>
